...
import static org.datatypeproject.Constants.LINE_SEPARATOR;
 ...
 * </ul>
 */
class PrimitiveHashMapOfIntKeyToCharArrayValue {
  ...
  private static class HashTable {
     ...
     *   <li>second index/dimension to get the value objects which are char-arrays.</li>
     * </ul>
     */
    private char[][][] values;
  }
  ...
  PrimitiveHashMapOfIntKeyToCharArrayValue() {
    final int initialCapacity = 20;
    this.hashTable = new HashTable();
    this.hashTable.keys = new int[initialCapacity][];
    this.hashTable.values = new char[initialCapacity][][];
  ...
  void put(final int key, final char[] value) {
    if (value == null) {
      return;
    }
  ...
  private void put(final int key, final char[] value, final HashTable hashTable) {
    int hashIndex = (key & 0x7FFFFFFF) % hashTable.keys.length;
    int[] bucket = hashTable.keys[hashIndex];
    if (bucket == null) {
      hashTable.keys[hashIndex] = new int[]{key};
      hashTable.values[hashIndex] = new char[][]{value};
      ++size;
    } else {
      int bucketIndex = 0;
  ...
  private void rehash() {
    ...
    newHashTable.values = new char[newCapacity][][];
    for (int i = 0; i < hashTable.keys.length; ++i) {
