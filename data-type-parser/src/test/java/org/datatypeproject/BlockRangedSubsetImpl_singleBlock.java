...
class BlockRangedSubsetImpl_singleBlock {
  ...
  void includeChar_singleByteTest(final char value) {
    ...
    assertThat(actual.getSingleByteCodePointRangeByBlock()).hasDimensions(1, 1);
    assertThat(actual.getSingleByteCodePointRangeByBlock()[0]).containsOnly(RangedSubsetUtils.rangeToChar(value, value));
  }
  ...
  void includeChars_singleByteTest(final SingleByteTestSource testSource) {
    ...
    assertThat(actual.getBlocks()).hasSize(1);
    assertThat(actual.getBlocks()).containsOnly(0x00);
    assertThat(actual.getSingleByteCodePointRangeByBlock()).hasDimensions(1, testSource.chars.length);
    assertThat(actual.getSingleByteCodePointRangeByBlock()[0]).containsOnly(testSource.expectedRanges);
  }
}
